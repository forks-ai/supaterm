...
struct TerminalSidebarDragSessionTests {
  ...
  func beginDragSeedsDragState() {
    let tab = TerminalTabItem(title: "Tab", icon: nil)
  ...
  func insertionIndexClampsToRowEdges() {
    let tabs = (1...3).map { TerminalTabItem(title: "Tab \($0)", icon: nil) }
  ...
  func reorderOffsetIsZeroWithoutActiveDrag() {
    let tabs = (1...2).map { TerminalTabItem(title: "Tab \($0)", icon: nil) }
    seedMeasuredFrames(tabs: tabs, zone: .regular)
  ...
  func reorderOffsetMakesRoomInTargetZone() {
    let regular = TerminalTabItem(title: "Regular", icon: nil)
    let pinned = (1...2).map { TerminalTabItem(title: "Pinned \($0)", icon: nil) }
