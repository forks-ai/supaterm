...
describe("worker", () => {
  ...
  it("serves agent detection rules with short revalidation and MIME sniffing disabled", async () => {
    const assetsFetch = vi.fn().mockResolvedValue(
      new Response("rule asset", {
        headers: {
          "cache-control": "public, max-age=14400",
          "content-type": "text/plain",
          etag: '"agent-detection-v1"',
        },
      }),
    );

    const response = await worker.fetch(
      new Request("https://supaterm.com/agent-detection/v1/rules.toml"),
      { ASSETS: { fetch: assetsFetch } as AssetBinding },
    );
    ...
    expect(response.headers.get("etag")).toBe('"agent-detection-v1"');
    await expect(response.text()).resolves.toBe("rule asset");
  });
  ...
  it("preserves conditional asset responses", async () => {
    const assetsFetch = vi.fn().mockResolvedValue(
      new Response(null, {
        status: 304,
        headers: { etag: '"rules-v1"' },
      }),
    );
    const request = new Request("https://supaterm.com/agent-detection/v1/rules.toml", {
      headers: { "If-None-Match": '"rules-v1"' },
    });
    ...
    expect(response.headers.get("etag")).toBe('"rules-v1"');
    expect(response.headers.get("cache-control")).toBe("public, max-age=300, must-revalidate");
    expect(response.headers.get("x-content-type-options")).toBe("nosniff");
    expect((await response.arrayBuffer()).byteLength).toBe(0);
  });

  it("preserves HEAD responses for agent detection assets", async () => {
    const assetsFetch = vi.fn().mockResolvedValue(
      new Response(null, {
        headers: {
          "content-length": "1024",
