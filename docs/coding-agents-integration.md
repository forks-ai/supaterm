...
- A `Stop` payload that carries `background_tasks` is also authoritative for registered children: subagent entries with status `running` keep their rows, and every other child row is removed. This clears stale children whose `SubagentStop` fired while Supaterm was not running, such as across an app restart that live agent processes survive. Workflow children are exempt. Claude registers a running workflow as one `workflow` entry and never lists the agents inside it, so the payload cannot speak for them; only `SubagentStop`, workflow completion, or `TaskStop` removes a workflow row.
