...
- `SubagentStart` and `SubagentStop` maintain scoped child-agent rows without allowing a child to replace the foreground root session. Claude writes `subagents/agent-<id>.meta.json` beside the session transcript when it spawns an agent; that file supplies the child's name, task description, and `taskKind`. The file lands moments after `SubagentStart`, so child tool hooks retry the read and fill the row once it exists. Names and tasks persist across turn boundaries while the agent keeps running, and child tool hooks cannot replace them with tool names. Claude child scopes do not independently tail the root transcript.
