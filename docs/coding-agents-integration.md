...
- Supaterm injects pane-local environment into terminal processes:
  ...
  - `SUPATERM_SURFACE_ID`
  - `SUPATERM_TAB_ID`
- Supaterm prepends the app's `Contents/MacOS` directory to pane `PATH`. The directory ships `sp` and `ap`, the coding-agents session picker built from the `ThirdParty/coding-agents-session-picker` submodule.
...
- `Notification` marks the tab as `needs input` only for `permission_prompt`, `idle_prompt`, and `elicitation_dialog`. An `idle_prompt` cannot replace running background work with `needs input`.
...
- `Stop` marks the tab as `idle` and stores the final assistant message as the latest tab notification when one is provided, unless the payload reports a running `background_tasks` entry or a pending `session_crons` entry.
- A `Stop` payload that carries `background_tasks` is also authoritative for children: subagent entries with status `running` keep their rows, and every other child row is removed. This clears stale children whose `SubagentStop` fired while Supaterm was not running, such as across an app restart that live agent processes survive. Claude registers a running workflow as one `workflow` entry and never lists the agents inside it, so workflow children are kept by their run instead: a workflow row survives while a `workflow` entry with its name is running, and the first `Stop` after the run leaves the list sweeps every row of that run, named or not.
...
- `SubagentStart` and `SubagentStop` maintain scoped child-agent rows without allowing a child to replace the foreground root session. Claude writes `subagents/agent-<id>.meta.json` beside the session transcript when it spawns an agent; that file supplies the child's name and task description. Workflow-tool subagents write it one level deeper, under `subagents/workflows/<run-id>/`, and their file carries neither name nor description — for those the row name comes from the workflow script persisted at `workflows/scripts/<name>-<run-id>.js`, and the row task comes from the spawn prompt, the first user message of the sibling `agent-<id>.jsonl`, truncated to 140 characters. The hook's `workflow-subagent` agent type is what marks a child as workflow-run; it arrives with `SubagentStart`, before any file does, and the panel groups those children under one row per run instead of showing the workflow name on each of them. That row carries the run name and its done-over-total count, and lists only the agents still working. The files land moments after `SubagentStart`, so child tool hooks retry the read and fill the row once they exist. Names and tasks persist while the agent keeps running, and child tool hooks cannot replace them. Child `PreToolUse` and `PostToolUse` hooks do supply the row's live detail — the tool name paired with the salient input, such as the Bash command or the file being read — so the row shows what the child is doing right now while the task remains the fallback before the first tool call. Claude child scopes do not independently tail the root transcript. Subagent-scoped events update only their child row: a subagent `Stop` never emits the turn-complete notification, and a subagent prompt never clears the root's recent notification.
- `SubagentStop` removes every finished child. A later `SubagentStart` for the same child adds it again. A finished workflow child is marked done instead of removed, so the run keeps its tally; the panel counts it and stops giving it a row. Workflow completion and `TaskStop` remove every child in that workflow run, covering failed children that do not emit `SubagentStop`.
- A new user turn drops the previous turn's children but keeps workflow children, because a workflow keeps running across the turns that follow the one that launched it.
