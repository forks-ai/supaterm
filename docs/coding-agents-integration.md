...
- `SubagentStart` and `SubagentStop` maintain scoped child-agent rows without allowing a child to replace the foreground root session. Claude writes `subagents/agent-<id>.meta.json` beside the session transcript when it spawns an agent; that file supplies the child's name, task description, and `taskKind`. Workflow-tool subagents write it one level deeper, under `subagents/workflows/<run-id>/`, and their file carries neither name nor description — for those the row task comes from the spawn prompt, the first user message of the sibling `agent-<id>.jsonl`, truncated to 140 characters. The files land moments after `SubagentStart`, so child tool hooks retry the read and fill the row once they exist. Names and tasks persist across turn boundaries while the agent keeps running, and child tool hooks cannot replace them with tool names. Claude child scopes do not independently tail the root transcript. Subagent-scoped events update only their child row: a subagent `Stop` never emits the turn-complete notification, and a subagent prompt never clears the root's recent notification.
- `SubagentStop` removes one-shot task subagents, but marks `in_process_teammate` children idle and keeps them listed — Claude fires the hook whenever a teammate finishes responding, yet the teammate stays resident and can run again. A later `SubagentStart` for the same child flips the row back to running.
