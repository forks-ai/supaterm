...
For login-shell panes, the app prepends its `Contents/MacOS` directory to `PATH`. The directory includes the matching `sp` CLI. Direct launches keep the caller's `PATH` unchanged.
