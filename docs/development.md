...
UI tests in `apps/mac/supatermUITests` control the shared macOS desktop, input focus, and pasteboards. Never run them locally. Push the branch, dispatch the UI-only CI scope, and watch the returned run:

```bash
branch="$(git branch --show-current)"
git push origin "$branch"
run_url="$(gh workflow run ci.yml --ref "$branch" -f scope=ui)"
gh run watch "${run_url##*/}" --exit-status
```

Full runs use five shards by default and accept `ui_shards` as an override. Set `ui_test` to an exact target, class, or method identifier to run it on one shard:

```bash
gh workflow run ci.yml --ref "$branch" -f scope=ui \
  -f ui_test=supatermUITests/SettingsUITests/testAboutShowsVersionAndUpdateControls
```
